...
void readcommandline(int argc, char** argv){
	...
	bool file = 0;
	// Read command line
	while ((c = getopt (argc, argv, "f:vy:m:p:r:hd:b:l:2:")) != -1){
		numopts++;
		switch (c) {
			case 'f':
				FILE_ARG = optarg;
...
#else
	init_register_access(top->v->RegRead->PhysRegFile, top->v->CommitRetire);
#endif
		...
		if(top->v->data_valid_fDC && top->CLK) {
			...
			if(top->v->write_2DC) {
				...
				case 2:
					...
					break;
				case 3:
					cachewrite << "Write3:\t  " << num2Str3((uint32_t)top->v->data_write_2DC)<<" at "<<num2Str((uint32_t)top->v->data_address_2DC)<<"\tcycle:"<<((uint32_t)CLOCK_COUNTER)<<endl;
					break;
				}
			}
			if(top->v->flush_2DC) {
...
//BREAKPOINT == top->v->Instr_address_2IM || 
			...
			if(((nextInstructionAddr < 0x400000) /*|| (nextInstructionAddr < 0x10000000 && elf_format==EXEC_TEXT)*/) && CLOCK_COUNTER>1){

...
static VMIPS_CommitRetire* _CR = NULL;

void init_register_access(VMIPS_PhysRegFile *prf, VMIPS_CommitRetire *cr){
	_PhysRegFile = prf;
	_CR = cr;
}

#define REG_FILE_VALID	(_PhysRegFile && _CR)
...
uint32_t register_arch_to_phys(uint32_t regno) {
	...
	return _CR->RRAT__DOT__regPtrs[regno];
#endif
}
